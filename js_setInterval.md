...
分析发现，请求上证 50 指数成分股的分钟级行情数据延时均值在 250ms 。而处理需要 300ms 。一个 TestCard 组件的定时函数执行完成需要 550ms 。

> TODO 原因分析 -> 需要理解 ts 的异步过程
...
观测发现，使用该方法后，定时取数方面能够保证每分钟每个卡片都正确的请求获取到分钟级行情数据。目前存在的问题有：1. 每个卡片取数是单独更新，而非全部卡片统一更新。
